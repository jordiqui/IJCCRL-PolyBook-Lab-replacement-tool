...
IJCCRL_POLYBOOK_VERSION = "0.1.0-p1"
PHASE_NAME = "POLYBOOK-P1"
...
def parse_book(path: Path) -> List[Record]:
    ...
    if len(data) % POLYGLOT_RECORD_SIZE != 0:
        raise ValueError(
            f"Invalid file size {len(data)} for {path}; expected multiple of {POLYGLOT_RECORD_SIZE} bytes"
        )
    recs = []
...
def analyze(recs: List[Record]) -> Dict:
    ...
    for i in range(1, len(recs)):
        if (recs[i - 1].key, recs[i - 1].move, recs[i - 1].weight, recs[i - 1].learn) > (
            recs[i].key,
            recs[i].move,
            recs[i].weight,
            recs[i].learn,
    ...
    return {
        ...
        "duplicate_full_records": [
            {"record": asdict(r), "count": c} for r, c in full_counter.items() if c > 1
        ],
        "duplicate_key_move_pairs": [
            {"key": k, "move": m, "count": c} for (k, m), c in keymove_counter.items() if c > 1
        ],
        "is_canonically_sorted": recs == sorted_recs,
...
def compare_books(old: List[Record], new: List[Record]) -> Dict:
    ...
    return {
        "added_records": sum((nc - oc).values()),
        "new": analyze(new),
        "old": analyze(old),
        "removed_records": sum((oc - nc).values()),
    }
...
def cmd_merge(args):
    ...
    merged, conflicts = merge_records([parse_book(p) for p in paths], args.policy)
    ...
    rep = {
        ...
        "output": {"path": str(out), "records": len(merged), "sha256": sha256_file(out)},
        "policy": args.policy,
    }
    write_report(args.json, rep)
...
def build_parser():
    ap = argparse.ArgumentParser(prog="ijccrl-polybook", description="IJCCRL PolyBook CLI")
    ...
    p.add_argument("--policy", choices=["aggregate-sum", "aggregate-max", "keep-duplicates"], required=True)
    ...
    p.add_argument("--max-ply", type=int, default=16)
    p.add_argument("--max-positions", type=int, default=100)
    p.add_argument("--max-moves-per-position", type=int, default=32)
    p.add_argument("--output-jsonl", required=True)
    p.add_argument("--json", required=True)
    p.set_defaults(func=eval_book)
    return ap
